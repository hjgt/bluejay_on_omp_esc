...
> 文档状态：2026-09-15。源码已迁移到 Bluejay 稳定版 **v0.21.0**；原厂BLHeli_S 16.7双份一致备份的反汇编已确认全部控制引脚、三相顺序、比较器和306ns死区。当前源码修正后的 `omp2` 尚待有许可的Keil独立重建，因此现阶段没有建议刷写的正式HEX。
...
| 可编译 | 使用有许可的 Keil 工具链从当前源码实际生成并校验 HEX | `omp1`已验证；修正后的`omp2`待独立重建 |
| 可上台架 | 固件映射和备份完成，发布版HEX及P1.3比例做电气复核 | 待`omp2`正式构建及上电复核 |
...
此前在 Apple Silicon Mac 的临时Wine/Keil环境中，`omp1`曾使用正式PK51许可完成实际汇编、链接和HEX生成；该临时环境后来已按要求移除。许可证没有写入项目源码或文档。当前修正后的`omp2`仍需重新建立工具链并从源码独立构建。
...
2026-09-09 已对 `omp1` 基线在当前工作区连续强制构建两次，结果完全一致。2026-09-15根据原厂反汇编把死区从5改为15，并让P2.0/C2D保持高阻，形成 `omp2` 候选：
...
当前机器上临时Keil环境已移除，因此 `omp2` 候选是在已验证的 `omp1` 编译输出上仅更新由上述源级常量决定的4个数据字节，并重新计算对应Intel HEX记录校验和。正式Release前应再用有许可的Keil从当前源码独立重建，确认逐字节一致；在此之前不要把候选当作量产镜像。

当前本地候选HEX的SHA-256如下，仅用于将来独立重建时做逐字节比对，不构成刷写许可：

```text
1d761b6bbb27ccf7521842d53dc3f04f8560b39e0556eb57f0aba204360e00df
```
...
> **当前暂停在“写入”之前：** 以下流程保留为正式`omp2`发布后的操作手册。在Release明确注明“有许可Keil从当前源码独立重建，且与候选逐字节一致”以前，只可执行连接、识别和读取，不要执行`Write`、`Erase MCU`或`Flash Other`。
...
5. 正式Release发布后，进入 `Write` 页，把该Release中的 `X_H_15_24_v0.21.0-omp2.hex` 拖入文件框，或点击文件框选择该HEX。
...
因此它适合已有可靠原厂备份、希望用GUI完成首次烧录的情况；若要严格验证写入后的Flash，使用方法C。
...
5. 正式Release发布后，确认原厂备份哈希和第4节门槛，再点击 `Flash Other`，手动选择该Release中的 `X_H_15_24_v0.21.0-omp2.hex`。
...
正式Release通过独立重建检查后，无论使用哪种方法，都只选择该Release中的以下文件，并以Release实际公布的SHA-256为准：

```text
X_H_15_24_v0.21.0-omp2.hex
候选预期SHA-256: 1d761b6bbb27ccf7521842d53dc3f04f8560b39e0556eb57f0aba204360e00df
```

方法C的写入和严格回读命令为：

```bash
python3 tools/verify_efm8_hex.py build/hex/X_H_15_24_v0.21.0-omp2.hex
...
python3 tools/verify_efm8_hex.py flashed_readback.hex --full-backup \
  --compare-programmed build/hex/X_H_15_24_v0.21.0-omp2.hex
```
...
- [x] 原厂16KiB双份备份完成且一致；
- [ ] 修正后的`omp2`由有许可的Keil从当前源码独立构建，并与候选逐字节一致；
- [ ] 正式Release HEX通过地址、标签、校验和及关键机器码检查；
- [ ] C2刷写后逐字节回读一致；
